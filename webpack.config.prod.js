...
module.exports = {
  ...
  output: {
    path: path.join(__dirname, config.assets.buildRoot),
  ...
  module: {
    loaders: [
      ...
      { test: /\.js$/, loader: 'babel', include: path.join(__dirname, 'client') },
      { test: /\.css$/, loader: ExtractTextPlugin.extract('style', 'css?modules&importLoaders=1&localIdentName=[name]__[local]___[hash:base64:5]!postcss?pack=default'), include: path.join(__dirname, 'client') },
